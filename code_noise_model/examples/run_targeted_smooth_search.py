"""Run robust search over smooth designs targeted at process-subtype recovery."""
...
from noise_model.search import generate_targeted_smooth_designs
...
def main() -> None:
    ...
    designs = generate_targeted_smooth_designs(n_trials=240, seed=2500)
    ...
    conservative.to_csv(OUTPUTS / "targeted_smooth_search_conservative.csv", index=False)
    binary.to_csv(OUTPUTS / "targeted_smooth_search_binary.csv", index=False)
    subtype.to_csv(OUTPUTS / "targeted_smooth_search_subtype.csv", index=False)
    predictions.to_csv(OUTPUTS / "targeted_smooth_search_predictions.csv", index=False)

    top_binary = conservative[conservative.false_process_target == 0.05].head(10)
    top_subtype = subtype.head(10)
    ...
    print("Top targeted smooth designs at false process <= 0.05:")
    print(top_binary.to_string(index=False))
    print("\nTop targeted smooth designs by process subtype:")
